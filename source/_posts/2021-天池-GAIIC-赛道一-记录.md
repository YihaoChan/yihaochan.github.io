...
3. 对损失函数手动分配权重：由于标签类别不平衡，最多的类别所对应数据数量有1846，最少的只有209，属于类别不平衡问题。因此考虑在训练的时候给损失函数直接设定一定的比例，比例大小和标签数量成反比，使得算法能够对小类数据具有更多的"注意力"。初赛时我做了这个尝试，不过效果不好。猜想：标签类别是否已经不平衡到需要手动分配权重的程度？也就是说，在这种情况下，手动分配权重是否有必要？；
...
1. 训练时间：Tesla V100单卡，全流程运行需要42小时左右；
2. 推理速度：Tesla V100单卡，推理速度320ms/条。
