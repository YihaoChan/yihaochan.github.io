...
1. 交替训练 & 持续学习：
   ...
   有两个队伍基于这两个思想采取了两种不同的策略：

   1. 交替训练任务1和任务2满足**任务自适应**的定义，因此，首先用任务1对预训练权重进行微调，之后将任务1的微调权重用于微调任务2，再反过来微调任务1，最终再微调任务2；
   2. 持续学习：首先用任务2对预训练权重进行微调，之后加载上一步的权重，对任务1和任务2同时训练。
